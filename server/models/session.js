...
Session.create = function (userId, callback) {
  ...
  Async.auto({
    ...
    newSession: ['keyHash', (done, results) => {
      const document = {
        userId: userId,
...
Session.findByCredentials = function (id, key, callback) {
  ...
  Async.auto({
    ...
    keyMatch: ['session', (done, results) => {
      if (!results.session) { return done(null, false); }
    ...
    if (err) { return callback(err); }
    if (results.keyMatch) { return callback(null, results.session); }
